...
// Solo Rider / Driver plan pricing — mirrors debit_wallet_for_subscription()'s
// actual pricing for the individual_rider plan (subscription_plans:
// monthly_price_ngn=2500, annual_price_ngn=25500; quarterly/biannual =
// monthly * cycle * discount). Keep in sync with that function/table if the
// price ever changes.
const SOLO_CYCLES = [
...
export default function RiderApp() {
  ...
  const loadRiderData = async () => {
    try {
      await loadRiderDataInner();
    } catch (e) {
      console.error('loadRiderData failed', e);
      toast.error('Could not load your rider dashboard. Check your connection and try again.');
    } finally {
      setLoading(false);
    }
  };

  const loadRiderDataInner = async () => {
