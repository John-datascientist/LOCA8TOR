...
import { ArrowLeft, Check, Loader2, Wallet as WalletIcon, Calendar, AlertCircle, Plus, XCircle } from 'lucide-react';
...
export default function Billing() {
  ...
  const [checkout, setCheckout] = useState<{ code: string; name: string; amount: number } | null>(null);
  ...
  const load = async () => {
    ...
    const [{ data: p }, { data: s }, { data: rider }, { data: wallet }] = await Promise.all([
  ...
  const subscribe = async (plan_code: string) => {
    ...
    setCheckout({ code: p.code, name: p.name, amount });
  };
  ...
  const subFor = (code: string) => (sub?.plan_code === code ? sub : undefined);
          ...
          <>
            ...
            {sub && sub.status !== 'cancelled' && (
              <div className="bg-card ring-1 ring-border rounded-lg p-4 flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold text-foreground">Active subscription: {sub.plan_code}</p>
...
function Section({ title, plans, cycle, subFor, onSubscribe, busy }: {
  ...
  subFor: (c: string) => BSub | undefined;
