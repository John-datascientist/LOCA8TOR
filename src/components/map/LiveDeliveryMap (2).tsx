...
export default function LiveDeliveryMap({
  ...
  const animPosRef = useRef({ lat: riderLat, lng: riderLng });
  ...
  useEffect(() => {
    ...
    // Smoothly interpolate the marker between the previous and new GPS
    // fix rather than jumping.
    if (animRafRef.current) cancelAnimationFrame(animRafRef.current);
    const start = animPosRef.current;
    const duration = 1500;
