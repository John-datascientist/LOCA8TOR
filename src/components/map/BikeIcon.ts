...
// Raw HTML versions of the icons above, for map engines that take a plain
// DOM element/HTML string directly instead of a Leaflet-specific icon
// object (e.g. MapLibre GL JS's Marker).
export function vehicleMarkerHtml(type: VehicleType | string | null | undefined): string {
  const key = (type as VehicleType) in VEHICLE_EMOJI ? (type as VehicleType) : 'bike';
  return vehicleSvg(key);
}
export const pickupMarkerHtml = PICKUP_SVG;
export const dropoffMarkerHtml = DROPOFF_SVG;
