...
import { Loader2, X, Building2, CreditCard, Copy, CheckCircle2, Clock } from 'lucide-react';
...
export default function FundWalletModal({ onClose }: { onClose: () => void }) {
  ...
  const payWithCard = async () => {
    ...
    setBusy(true);
    const { data, error } = await supabase.functions.invoke('stripe-fund-wallet', {
      body: { amount_ngn: amount },
    });
    setBusy(false);
    if (error || (data as any)?.error || !(data as any)?.url) {
      toast({ title: 'Could not start card payment', description: (data as any)?.error || error?.message, variant: 'destructive' });
      return;
    }
        ...
        {step === 'form' && (
          <>
            ...
            <div>
              ...
              <div className="mt-1 grid grid-cols-2 gap-2">
                <button onClick={() => setMethod('bank')}
                  className={`flex items-center justify-center gap-2 p-3 rounded-md text-xs font-semibold ring-1 ${method === 'bank' ? 'ring-primary bg-primary/10 text-primary' : 'ring-border bg-secondary text-muted-foreground'}`}>
                  <Building2 className="w-4 h-4" /> Bank Transfer
                </button>
                <button onClick={() => setMethod('card')}
                  className={`flex items-center justify-center gap-2 p-3 rounded-md text-xs font-semibold ring-1 ${method === 'card' ? 'ring-primary bg-primary/10 text-primary' : 'ring-border bg-secondary text-muted-foreground'}`}>
              ...
              {method === 'card' && (
                <div className="mt-3">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">Charged in USD at today's rate via Stripe — card, Apple Pay & Google Pay accepted. Your wallet is credited instantly.</p>
                </div>
              )}
            </div>
            ...
            {method === 'bank' ? (
              ...
              <button onClick={payWithCard} disabled={busy}
                className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground font-heading font-bold py-2.5 rounded-md hover:bg-primary/90 disabled:opacity-50">
                {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : `Pay ₦${walletCredit.toLocaleString()} with card`}
              </button>
            )}
          </>
        )}
