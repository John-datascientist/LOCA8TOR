...
function PostcodeNavigationViewInner({ lat, lng, postcode }: Props) {
  ...
  // Temporary on-screen GPS diagnostic (?debug=1): the map/instruction
  // banner froze on a real device with no console access to inspect why,
  // so this surfaces raw watchLivePosition callback activity directly in
  // the UI instead. Remove once the freeze's root cause is confirmed.
  const gpsDebug = typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('debug');
  const [gpsLog, setGpsLog] = useState<{ fixes: number; lastAt: number | null; lastPos: { lat: number; lng: number } | null; lastError: number | null }>(
    { fixes: 0, lastAt: null, lastPos: null, lastError: null },
  );
    ...
    (async () => {
      stop = await watchLivePosition(
        (pos) => {
          ...
          setRiderPos(pos);
          setGpsLog((g) => ({ ...g, fixes: g.fixes + 1, lastAt: Date.now(), lastPos: pos }));
        },
        () => {
          if (cancelled) return;
          setGpsError(true);
          setGpsLog((g) => ({ ...g, lastError: Date.now() }));
        },
      );
    })();
  ...
  // Forces the debug panel's "Ns ago" to tick even when no new GPS fix
  // arrives — that stall is exactly the thing being diagnosed.
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 relative min-h-[400px]">
        {gpsDebug && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 bg-black/85 text-white text-[10px] font-mono px-3 py-2 rounded-lg leading-relaxed pointer-events-none">
            <div>fixes: {gpsLog.fixes}{gpsLog.lastError ? ' · ERROR fired' : ''}</div>
            <div>last fix: {gpsLog.lastAt ? `${Math.round((Date.now() - gpsLog.lastAt) / 1000)}s ago` : 'never'}</div>
            <div>pos: {gpsLog.lastPos ? `${gpsLog.lastPos.lat.toFixed(6)}, ${gpsLog.lastPos.lng.toFixed(6)}` : '—'}</div>
          </div>
        )}
        {riderPos ? (
          <LiveDeliveryMap
