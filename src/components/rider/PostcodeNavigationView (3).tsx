...
function PostcodeNavigationViewInner({ lat, lng, postcode }: Props) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Map area — fills all remaining space below the header. Once a trip
          has started, navigation happens in Google Maps (opened in a new
          tab), so the in-app map is replaced by a simple confirmation
          instead of continuing to render/track it. The sheet below floats
          on TOP of the map as an overlay instead of squeezing its container. */}
        ...
        {started ? (
          <div className="w-full h-full flex items-center justify-center bg-muted">
            <div className="text-center p-6 space-y-2 max-w-xs">
              ...
              <p className="text-sm text-muted-foreground">Your {activeVehicle?.label.toLowerCase()} trip opened in Google Maps.</p>
            </div>
          </div>
        ) : riderPos ? (
          <LiveDeliveryMap
