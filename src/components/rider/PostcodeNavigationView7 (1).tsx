...
import { Loader2, MapPin, ArrowLeft, ExternalLink, Copy } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 relative">
        ...
        {/* Bottom sheet — overlays the map, never resizes it. Kept as short
            as possible (single horizontal row + one button) so it doesn't
            eat most of the screen and crowd out the map above it. */}
        <div className="absolute bottom-0 left-0 right-0 z-10 rounded-t-2xl shadow-[0_-4px_20px_rgba(0,0,0,0.15)] bg-card px-3 pt-3 pb-3 space-y-2.5">
            ...
            {VEHICLE_OPTIONS.map((v) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 min-w-[68px] flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-colors ${active ? 'bg-primary/10 ring-2 ring-primary' : 'bg-secondary/60 hover:bg-secondary'}`}
                >
                  <span className="text-xl leading-none">{v.emoji}</span>
                  <span className="text-[10px] font-heading font-bold text-foreground">{v.label}</span>
                  <span className="text-[9px] text-muted-foreground">
                    {eta ? (eta > 0 ? `${eta} min` : '<1 min') : '—'}
                  </span>
                </button>
              );
            })}
