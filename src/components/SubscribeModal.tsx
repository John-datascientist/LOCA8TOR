...
import { Loader2, X, CreditCard, ExternalLink, CheckCircle2, Wallet, Info, KeyRound } from 'lucide-react';
          ...
          {activated ? (
            <div className="space-y-3 text-center py-4">
              ...
              {walletOnly ? (
                <>
                  <button
                    onClick={() => { onClose(); navigate('/api-keys'); }}
                    className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground font-heading font-bold py-2.5 rounded-md text-sm"
                  >
                    <KeyRound className="w-4 h-4" /> Get your API key
                  </button>
                  <button onClick={onClose} className="w-full text-muted-foreground text-xs font-semibold py-1.5">
                    I'll do this later
                  </button>
                </>
              ) : (
