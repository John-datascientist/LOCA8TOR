...
BEGIN
  ...
  SELECT (email_confirmed_at IS NOT NULL) INTO _verified
  FROM auth.users WHERE id = _uid;

  IF NOT COALESCE(_verified, false) THEN
    RETURN jsonb_build_object('success', true, 'credited', 0, 'pending_reason', 'email_unverified');
  END IF;
  ...
  IF _rr.id IS NOT NULL AND _rr.referrer_user_id <> _uid THEN
    ...
    UPDATE public.rider_referrals
       SET status = 'completed', credits_earned = _amount, credited_at = now(),
           first_delivery_at = COALESCE(first_delivery_at, now())
     WHERE id = _rr.id;

    SELECT referral_code INTO _code FROM public.user_referral_balances
    WHERE user_id = _rr.referrer_user_id LIMIT 1;
  ...
  WHERE referred_user_id = _uid AND credited_at IS NULL AND status <> 'rejected'
  ...
  IF _r.id IS NOT NULL THEN
    ...
    IF _referrer_user IS NOT NULL AND _referrer_user <> _uid THEN
      ...
      SELECT referral_code INTO _code FROM public.user_referral_balances
      WHERE user_id = _referrer_user LIMIT 1;
      ...
      UPDATE public.referrals
         SET credited_at = now(), credits_earned = _amount, status = 'completed'
       WHERE id = _r.id;
