...
BEGIN
  ...
  SELECT count(*) INTO _total_riders
  FROM public.riders
  WHERE account_type = 'rider';

  SELECT count(*) INTO _total_businesses
  FROM public.riders
  WHERE account_type = 'business';

  SELECT count(*) INTO _total_postcodes
  FROM public.postcodes;

  SELECT coalesce(sum(amount) FILTER (WHERE amount > 0), 0)::bigint INTO _total_quiz_payout
  FROM public.quiz_balance_ledger;

  SELECT coalesce(sum(amount) FILTER (WHERE status = 'pending'), 0)::bigint INTO _pending_payout
  FROM public.withdrawals;

  SELECT
    coalesce(sum(total_earned), 0)::bigint,
    coalesce(sum(balance), 0)::bigint,
    coalesce(sum(total_referrals), 0)::bigint
...
REVOKE ALL ON FUNCTION public.admin_get_platform_overview(text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_get_platform_overview(text, text) TO authenticated, anon, service_role;