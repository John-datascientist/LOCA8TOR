...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: plan } = await admin.from('subscription_plans')
      .select('code, category, monthly_price_ngn, annual_price_ngn, is_active').eq('code', plan_code).maybeSingle()
    ...
    const isApiPlan = (plan as any).category === 'api'
    ...
    if (isApiPlan) {
      // API plans have their own subscription slot (api_subscriptions),
      // independent of the fleet trial tracked on riders.trial_ends_at —
      // switching API plans always gets a fresh 7-day trial rather than
      // inheriting whatever's left of a fleet trial.
      await admin.from('api_subscriptions')
        .update({ status: 'cancelled', auto_renew: false, cancelled_at: now.toISOString() })
        .eq('business_user_id', user.id)
        .in('status', ['active', 'past_due'])

      trialEnd = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000)

      const { data: inserted, error: insErr } = await admin.from('api_subscriptions').insert({
